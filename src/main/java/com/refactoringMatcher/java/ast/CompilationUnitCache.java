	...
	public CompilationUnit getCompilationUnit(ITypeRoot iTypeRoot) {
		if(iTypeRoot instanceof IClassFile) {
			...
			if(iTypeRootList.contains(iTypeRoot)) {
				...
				int maximumCacheSize = 10;//store.getInt(PreferenceConstants.P_PROJECT_COMPILATION_UNIT_CACHE_SIZE);
				System.out.println("[TODO PDGCFG] CompilationUnit maximumCacheSize set as constant 10, possibly no usage, later remove safely");
